...
def seeds_change():
    ...
    app.logger.info("refresh-subset request received")
    ...
    if subsetUpdateTimestamp is None or seedsUpdateTimestamp > subsetUpdateTimestamp:
        ...
        f = Factor(ontologySource, SUBSET_BASE_URI, True, log=lambda msg: app.logger.info(msg))
        f.prime(seedList=seeds).writeSubset(destination, SUBSET_FORMAT)

    else:

        app.logger.info("Subset is up-to-date with seeds, no re-generation of subset is necessary")
